...
const DiploBotAvatar = ({ className = 'w-10 h-10' }: { className?: string }) => (
  <div
    className={`${className} rounded-full bg-gradient-to-br from-mun-green to-green-400 flex-shrink-0 flex items-center justify-center text-white font-bold text-base shadow-md`}
  >
    D
  </div>
);
...
const UserAvatar = ({ className = 'w-10 h-10' }: { className?: string }) => (
  <div
    className={`${className} rounded-full bg-gray-300 flex-shrink-0 flex items-center justify-center`}
  >
    <svg
...
const Chatbot = () => {
  ...
  const handleSend = async (messageToSend?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: aiMessageId, sender: 'ai', text: '__GENERATING__' },
      ]);
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.text !== '__GENERATING__'),
        errorMessage,
      ]);
    } finally {
      setIsLoading(false);
  ...
  const renderMarkdown = (text: string) => {
    if (typeof window !== 'undefined' && (window as any).marked) {
      let rawMarkup = (window as any).marked.parse(text, {
        breaks: true,
        gfm: true,
      });
  ...
  return (
    <>
      ...
      <button
        ...
        } w-16 h-16 bg-mun-green rounded-full text-white flex items-center justify-center shadow-2xl shadow-mun-green/30 z-40 transition-all duration-300 hover:scale-110 hover:bg-green-500 ${
      ...
      <div
        className={`fixed ${
          isMobile ? 'bottom-4 right-4' : 'bottom-8 right-8'
        } z-50 w-[min(calc(100vw-2rem),400px)] md:w-[450px] lg:w-[500px] h-[min(80vh,700px)] flex flex-col transition-all duration-500 ease-in-out ${
          isOpen
        ...
        <div className="bg-white rounded-2xl w-full h-full flex flex-col shadow-2xl border border-gray-200/80">
          ...
          <header className="flex justify-between items-center p-5 border-b border-gray-200/80 bg-gray-50/70 rounded-t-2xl flex-shrink-0">
            <div className="flex items-center gap-4">
              <DiploBotAvatar />
              <h3 className="font-bold text-xl text-mun-dark-text">
                DiploBot, your MUN Helper
              </h3>
            </div>
            <button
              onClick={handleToggle}
              className="text-gray-500 hover:text-mun-dark-text transition-colors"
            ...
            <div className="flex flex-col gap-5">
              {messages.map((msg) => {
                const isLastAiMessage =
                  lastMessage?.sender === 'ai' && msg.id === lastMessage.id;
                const showCursor =
                  isLoading &&
                  isLastAiMessage &&
              ...
              <div className="pb-3 flex flex-wrap gap-2">
                {suggestions.map((s) => (
                  <button
                    ...
                    className="px-3.5 py-2 bg-white border border-gray-300 rounded-full text-sm text-mun-dark-text hover:bg-mun-soft-green hover:border-mun-green/50 transition-all duration-200"
                  >
                    {s}
                  </button>
                ))}
